...
def generate_children_dynasty(self, END_YEAR):
    ...
    # mother’s age at first childgjhc 
    ...
    number_of_children_attempted = random.choices(
        population=list(constants

def generate_children_family(self, END_YEAR):
    # father–mother age offset
    self.age_offset = random.choices(
        population=list(constants.PD_FATHER_AVERAGE_AGE_OFFSET.keys()),
        weights=list(constants.PD_FATHER_AVERAGE_AGE_OFFSET.values()),
...
class CK3Male:
    def __init__(self, parent_name, birth_position, birth_year, START_FAMILY_GEN_OFFSET, END_DATE, generation_type=GenerationType.FAMILY):
        ...
        self.is_living = self.death_year > END_YEAR

        self.children = generate_children_dynasty(self, END_YEAR) if generation_type == GenerationType.DYNASTY else self.generate_children_family(END_YEAR)
...
class CK3Female:
    def __init__(self, parent_name, birth_position, birth_year, END_YEAR):
        self.name = f"{parent_name}_Daughter{birth_position}"
        self.birth_year = birth_year
    ...
    def __str__(self):
        return f"FemalePerson(name={self.name}, birth_year={self.birth_year}, death_year={self.death_year}, is_living={self.is_living})"